...
public class VkTests extends BaseTest{
	...
	@Test
	public void makeAPostWithPhoto() throws IOException, InterruptedException{
		login = new LoginPage(driver);
		userProfile = login.successfullyLogin("TestAllMe@mail.ru", "TestAllMe2018");
		userProfile.goToMyProfile();
		int numberOfPosts = userProfile.postsCount();
		userProfile.choosePhotoForPost();
		userProfile.clickOnPostButton();
		Assert.assertEquals(numberOfPosts + 1, userProfile.postsCount());
	}
}
